...
public enum TinyMoon {
  ...
  public struct Moon: Hashable {
    init(date: Date) {
      ...
      // TODO: update `daysUntilFullMoon` and `daysUntilNewMoon`
      ...
      self.exactMoonPhase = Moon.exactMoonPhase(phaseFraction: phaseFraction)
    ...
    /// The moon phase for a specific date, prioritizing major phases.
    ///
    /// Use this property when you need a general understanding of the moon's phase for a day, especially when emphasizing the major phases is important for your application's context.
    ///
    /// If one of the major moon phases (new moon, first quarter, full moon, last quarter) occurs at any time within the 24-hour period of the specified date, this property will reflect that major moon phase.
    ///
    /// If no major moon phase occurs within the date's 24 hours (starting at 00:00 and ending at 23:59), the property will represent the moon phase closest to the specified date and time.
    ///
    /// - Note: Unlike `exactMoonPhase`, this property will prioritize major moon phases occurring at any point within a 24-hour period.
    public let moonPhase: MoonPhase
    public let name: String
    public let emoji: String
    
    /// The exact moon phase for a specific date and time.
    ///
    /// This property represents the precise moon phase for a given date and time, without prioritizing major moon phases over others. It provides a more detailed and accurate representation of the moon's phase, suitable for applications requiring precise lunar data.
    ///
    ...
    ///
    /// For example, given that the full moon occurs on `August 19, 2024 at 13:25 UTC` and the date we query for is `August 19, 2024 at 00:00 UTC`, this property will return `.waxingGibbous` because that is a more accurate representation of the moon phase at `00:00 UTC` time.
    ...
    internal static func daysUntilFullMoon(julianDay: Double) -> Int {
      ...
      // Keep `exactMoonPhase`?
      let moonPhase = TinyMoon.Moon.exactMoonPhase(phaseFraction: moonPhaseFraction)
